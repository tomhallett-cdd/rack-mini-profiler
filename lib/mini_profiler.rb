...
module Rack
  class MiniProfiler
    ...
    def user(env)
      val = @config.user_provider.call(env)
      log_it(env, "USER_PROVIDER", user_identity: val)
      val
    end
    ...
    def call(env)
      ...
      if skip_it || (
        ...
        log_it(env, "PRE_AUTH_OR_NO_COOKIE")
      ...
      if path.start_with? @config.base_url_path
        ...
        case file_name
        when 'results'
          log_it(env, "SERVE_RESULTS")
          return serve_results(env)
        when 'snapshots'
          log_it(env, "SERVE_SNAPSHOT")
          self.current = nil
          return serve_snapshot(env)
        when 'flamegraph'
          log_it(env, "SERVE_FLAMEGRAPH")
          return serve_flamegraph(env)
        end

        log_it(env, "SERVE_FILE")
      ...
      log_it(env, "HANDLE_COOKIE_BOTTOM")
